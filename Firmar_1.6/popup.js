$(function () {
    ...
    var init = function () {
        ...
        $("#savechanges").click(function () {
            ...
            if (entrada !== window.localStorage.getItem("xetid-extension-sign-begin") || $("input[name=randomize-ent]").val() !== window.localStorage.getItem("xetid-extension-sign-randomize-ent")) {
                ...
                if (isNaN(randomize) || randomize < 1)
					...
					chrome.runtime.sendMessage({funct: "generateNextEnt"}, function () {
						loadLogs();
                });
            }
            ...
            if (salida !== window.localStorage.getItem("xetid-extension-sign-end") || $("input[name=randomize-sal]").val() !== window.localStorage.getItem("xetid-extension-sign-randomize-sal")) {
                ...
                chrome.runtime.sendMessage({funct: "generateNextSal"}, function () {
					loadLogs();
                });
            }
        ...
        $("#toglestart").click(function () {
            if (window.localStorage.getItem("xetid-extension-sign-starttogle"))
                chrome.runtime.sendMessage({funct: "stopService"}, function (response) {
					if (!response.status) {
                ...
                chrome.runtime.sendMessage({funct: "startService"}, function (response) {
					if (response.status) {
    ...
    var systemLog = function (log) {
        chrome.runtime.sendMessage({funct: "systemLog", log: log}, function (response) {
			loadLogs();
        });
    };
    var clearLogs = function () {
        chrome.runtime.sendMessage({funct: "clearLogs"}, function (response) {
			loadLogs();
        });
    };
